...
export default (props: any) => {
    ...
    const onSubmit = async () => {
        ...
        const mintFee = await rpcRouterContract.fixedAmountFee(Contracts.PlatwinMEME2[process.env.APP_CHAIN_ID])
        console.log('mintFee', mintFee)
        if (mintFee[0].lt(0)) {
            const allowance = await cashContract.allowance(account, Contracts.RPCRouter[process.env.APP_CHAIN_ID])
            console.log('allowance', allowance)

            if (allowance.lt(mintFee)) {
                await cashContract.approve(Contracts.RPCRouter[process.env.APP_CHAIN_ID], ethers.constants.MaxUint256)
            }
        }
    ...
    return <div className="mint-container">
        ...
        <Button type="primary" onClick={onSubmit} className="btn-submit">Submit</Button>
    </div>
}